...
class Database {
    constructor() {
        ...
        firebase.initializeApp({
            ...
            databaseURL:
                "https://role-bot-v2-default-rtdb.europe-west1.firebasedatabase.app/",
        });
    ...
    cacheLocalMemberData(guildId, memberId, field, value) {
        ...
        if (!value && value !== null) {
            console.warn(
                `Are you sure you want to set ${guildId}/${memberId}/${field} as ${value}?`
            );
    ...
    readMemberData(guildId, memberId, field) {
        ...
        return new Promise((resolve, reject) => {
            try {
                if (
                    field
                        ? this.dbLocal[guildId][memberId][field]
                        : this.dbLocal[guildId][memberId]
                ) {
                ...
                ref.once(
                    ...
                    (err) => {
                        console.error(
                            `Error reading /${guildId}/${memberId}:\n ${err}`
                        );
                        reject(err);
                    }
                );
